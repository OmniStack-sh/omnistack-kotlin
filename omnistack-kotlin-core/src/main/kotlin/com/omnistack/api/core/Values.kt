...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw OmnistackInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw OmnistackInvalidDataException("'${name}' is not set")
            is JsonNull -> throw OmnistackInvalidDataException("'${name}' is null")
            else -> throw OmnistackInvalidDataException("'${name}' is invalid, received ${this}")
        }
